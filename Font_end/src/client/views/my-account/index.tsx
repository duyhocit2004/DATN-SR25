...
//             ({ getFieldValue }) => ({
//               validator(_, value) {
//                 const old = getFieldValue("oldPassword");
//                 if (!old && !value) return Promise.resolve(); 
//                 if (old && !value) return Promise.reject(new Error("Vui lòng nhập mật khẩu mới"));
//                 if (value.length < 6) return Promise.reject(new Error("Tối thiểu 6 ký tự"));
...
//                 if (!newPass && !value) return Promise.resolve(); 
//                 if (newPass !== value) return Promise.reject(new Error("Mật khẩu không khớp"));
//                 return Promise.resolve();
//               },
//             }),
//           ]}
...
const MyAccount: React.FC = () => {
  ...
  const onUpdate = async (values: any) => {
    setLoading(true);
    try {
      const updatedData: Partial<IFormData & { oldPassword?: string; newPassword?: string }> = {
        id: formData.id,
        name: values.name,
        phoneNumber: values.phoneNumber,
        email: values.email,
