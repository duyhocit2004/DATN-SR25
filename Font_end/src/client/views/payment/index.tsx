    ...
    try {
      if (token) {
        ...
        const hasError = results.some(
          (result) => result && result.status !== HttpCodeString.SUCCESS
        );

        if (hasError) {
          throw new Error("Failed to clear some cart items via API");
        }
        ...
        setSelectedItems([]);
        dispatch(
          dispatchAction("cart/setCartCount", cartList.length - selectedItems.length)
        );
        showToast({
        ...
        setCartList((prev) =>
          prev.filter(
            (item) =>
              !selectedItems.includes(`${item.productId}-${item.size}-${item.color}`)
          )
        );
        setSelectedItems([]);
      ...
      throw error; // Ném lỗi để ngăn thanh toán hoàn tất nếu xóa giỏ hàng thất bại
    }
  };
