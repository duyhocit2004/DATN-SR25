  ...
  const clearCartItem = async () => {
    try {
      if (token) {
        ...
        if (response.status === HttpCodeString.SUCCESS) {
          setCartItem(null);
          // Gọi API để lấy lại giỏ hàng và tính số lượng
          const cartResponse = await cartApi.getCartByUserId();
          const cartCount = cartResponse.status === HttpCodeString.SUCCESS
        ...
        localStorage.setItem("cart", JSON.stringify(updatedCart));
        setCartItem(null);
        // Đếm số lượng sản phẩm trong giỏ hàng từ localStorage
